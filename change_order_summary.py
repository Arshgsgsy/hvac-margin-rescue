...
OUTPUT_FILE = BASE_DIR / "output_summaries" / "change_summary.csv"
...
COPY (
    SELECT
        ...
        SUM(CASE WHEN status = 'Approved'
            THEN TRY_CAST(amount AS DOUBLE) ELSE 0 END) AS approved_value,

        SUM(CASE WHEN status = 'Rejected'
            THEN TRY_CAST(amount AS DOUBLE) ELSE 0 END) AS rejected_value,
